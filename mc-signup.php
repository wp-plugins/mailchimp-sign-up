...
die();
	...
	wp_enqueue_script( 'mcs_script', plugins_url( 'assets/js/mc_subscribe.js', __FILE__), array(), false, true );
   }
...
?>
 <div style="margin:0px 0px 0px 10px;">
       ...
       <h2>MailChimp API Settings:</h2>
		...
		<div class="mcs_style_picker">	
		<label for="mcs_form_border_color">Field Form BG Color:</label> 
        ...
        <br /><br /><hr /> 
        <div style="font-size:11px; margin-top:10px;"><p><img src="<?php  echo plugins_url( 'assets/images/jp_logo_orange.png',__FILE__); ?>" style="float:left"/> MailChimp sign up plugin by <a href="http://jasonprescher.com/" style="color:#D16249; text-decoration:none; font-style:italic">Jason Prescher</a></p></div>
        </div> 
        </div> 
<?php 
} 
...
function mcs_header_output() {
	      ?>
	      <!-- Mail Chimp Sign Up CSS--> 
	      <style type="text/css">
	           <?php self::msc_generate_css('.mcForm input[type=submit],.mcForm input[type=button],.mcForm input[type=text]', 'border', 'mcs_border_size', '','px solid'); ?>
	           <?php self::msc_generate_css('.mcForm input[type=submit],.mcForm input[type=button]', 'background-color', 'mcs_button_color', ''); ?>
	           <?php self::msc_generate_css('.mcForm input[type=submit],.mcForm input[type=button]', 'color', 'mcs_button_text_color', ''); ?>
	           <?php self::msc_generate_css('.mcForm input[type=submit],.mcForm input[type=button],.mcForm input[type=text]', 'margin', 'mcs_margin_size', '','px'); ?>
	           <?php self::msc_generate_css('.mcForm input[type=text]', 'padding', 'mcs_padding_size', '','px'); ?>
	           <?php self::msc_generate_css('.mcForm input[type=submit],.mcForm input[type=button]', 'padding', 'mcs_padding_size', '','px'); ?>

	           <?php self::msc_generate_css('.mcForm input[type=submit]:hover,.mcForm input[type=button]:hover', 'background-color', 'mcs_button_hover_color', ''); ?>
	           <?php self::msc_generate_css('.mcForm input[type=submit],.mcForm input[type=button]', 'border-color', 'mcs_button_border_color', ''); ?>
			   <?php self::msc_generate_css('.mcForm input[type=submit]:hover,.mcForm input[type=button]:hover', 'border-color', 'mcs_button_hover_color', ''); ?>
	          
	           <?php self::msc_generate_css('.mcForm input[type=text]', 'color', 'mcs_form_text_color', ''); ?>
	           <?php self::msc_generate_css('.mcForm input[type=text]', 'background-color', 'mcs_form_background_color', ''); ?>
	           <?php self::msc_generate_css('.mcForm input[type=text]', 'border-color', 'mcs_form_border_color', ''); ?>
...
function msc_generate_css( $selector, $style, $mod_name, $prefix='', $postfix='', $echo=true ) {
      $return = '';
...
add_action( 'wp_head' , array( 'msc_customize' , 'mcs_header_output' ) );
